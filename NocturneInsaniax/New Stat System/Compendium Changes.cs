        ...
        private class PatchCompendiumPrice
        {
            public static void Prefix(ref int StartNo)
            {
                if (StartNo == 18)
                {
                    ...
                    // If enough macca post-discount but not pre-discount (and stock not full and not already in stock and also either has a level lower than or equal to Demi-Fiend or NG+ is active)
                    if ((pelem.level <= dds3GlobalWork.DDS3_GBWK.unitwork[0].level || EventBit.evtBitCheck(2208)) && price > 0 && dds3GlobalWork.DDS3_GBWK.maka >= price && datCalc.datCheckStockFull() == 0 && datCalc.datSearchDevilStock(pelem.id) == -1)
                    {
                        StartNo = 17;
                    }
                }
